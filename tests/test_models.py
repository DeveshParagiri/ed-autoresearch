...
class ModelRecordTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.registry = tomllib.loads(
            (cls.model_root / "registry.toml").read_text()
        )

    def test_model_record_has_no_wrapper_directory(self) -> None:
        self.assertFalse((self.model_root / "other-models").exists())
        self.assertFalse((self.model_root / "lineage").exists())
        for name in (
            "README.md",
            "registry.toml",
            "parameter-inventory.csv",
            "parameters",
            "commit-artifacts",
            "upstream",
            "stock-ed",
        ):
            self.assertTrue((self.model_root / name).exists(), name)

    def test_registry_names_every_retained_model_variant(self) -> None:
    ...
    def test_parameter_inventory_covers_every_retained_json(self) -> None:
        parameter_root = self.model_root / "parameters"
        files = sorted(parameter_root.rglob("*.json"))
        with (self.model_root / "parameter-inventory.csv").open(newline="") as handle:
    ...
    def test_selected_model_map_overview_is_not_squashed(self) -> None:
        path = (
            self.root
            / "research"
            / "experiments"
            / "experiment.models-a-i-reproduction"
            / "figures"
            / "models-a-i-gfed5-mean-maps.png"
        )
        header = path.read_bytes()[:24]
        self.assertEqual(header[:8], b"\x89PNG\r\n\x1a\n")
        self.assertEqual(struct.unpack(">II", header[16:24]), (1800, 3200))

    def test_git_bundle_is_complete_and_valid(self) -> None:
        bundle = self.model_root / "upstream" / "ed-autoresearch.bundle"
    ...
    def test_commit_history_archive_matches_every_audited_ref(self) -> None:
        completed = subprocess.run(
            [sys.executable, "scripts/sync_models.py"],
            cwd=self.root,
            capture_output=True,
            text=True,
            check=False,
        ...
        self.assertIn("Model artifact coverage: PASS", completed.stdout)

        artifact_root = self.model_root / "commit-artifacts"
