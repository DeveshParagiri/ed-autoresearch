"""
GFED5-targeted fFire scoring.

  1. Compute per-cell EF from GFED5: EF = sum_t(GFED5_fFire) / sum_t(GFED5_BA_per_sec)
  2. Apply: pred_fFire = our_BA × EF
  3. Score against GFED5 fFire reference via ILAMB

Writes to ilamb/MODELS_LEADERBOARD_FFIRE_GFED5/{model}/fFire.nc.
"""
...
REF_FFIRE = REPO / "ilamb_ref_official" / "DATA" / "fFire" / "GFED5" / "fFire.nc"
REF_BA = REPO / "ilamb_ref_official" / "DATA" / "burntArea" / "GFED5" / "burntArea.nc"
SEC_PER_MONTH = (365.25 / 12) * 86400.0


def compute_per_cell_EF():
    ff = xr.open_dataset(REF_FFIRE)["fFire"].values  # kg/m^2/s
    ba_pct = xr.open_dataset(REF_BA)["burntArea"].values  # % per month
    ...
    den = np.nansum(ba_frac_per_sec, axis=0)
...
def make_ffire_for_model(model_name, EF, target_lat, target_lon):
    ...
    out_dir = REPO / "ilamb" / "MODELS_LEADERBOARD_FFIRE_GFED5" / model_name
    out_dir.mkdir(parents=True, exist_ok=True)
    out_p = out_dir / "fFire.nc"

    out_ds = xr.Dataset(
        {"fFire": (("time", "lat", "lon"), pred_ffire,
                    {"units": "kg m-2 s-1", "standard_name": "fire_carbon_flux",
                     "long_name": "Fire carbon emission flux"})},
        coords={"time": ds.time, "lat": target_lat, "lon": target_lon},
        attrs={"title": f"{model_name} fFire (S=0, GFED5-derived per-cell EF)",
               "method": "fFire = burnedArea * EF; EF = sum_t(GFED5_fFire) / sum_t(GFED5_BA_per_sec)",
               "reference_for_EF": "GFED5",
               "Conventions": "CF-1.7"})
