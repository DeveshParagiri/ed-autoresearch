"""
Multi-objective Model C retune via NSGA-II. 12 params, four ILAMB-proxy
objectives (Bias, RMSE, Seasonal, Spatial). Uses Lei's NC inputs (coupled-
consistent).
"""
...
def main():
    ...
    ap.add_argument("--out", default=str(REPO / "models" / "C-multiobj"))
    args = ap.parse_args()

    out_dir = Path(args.out); out_dir.mkdir(parents=True, exist_ok=True)
    print("Model C multi-objective NSGA-II refit on Lei's NC")
    ...
    print(f"  overall={best.user_attrs['overall_proxy']:.4f}")
    ...
    json.dump({"model": "Model C, multi-objective NSGA-II refit on Lei NC",
               "train_window": [2001, 2010], "params": best.params},
              open(out_dir / "params.multiobj-best.json", "w"), indent=2)
    json.dump({"n_trials": len(study.trials), "wall_seconds": dt, "best_trial": best.number,
              ...
              open(out_dir / "refit_multiobj_summary.json", "w"), indent=2)
