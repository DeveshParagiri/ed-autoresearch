#!/usr/bin/env python3
"""Reproduce the retained ED-Fire models A through I and verify their scores.

The original generators used benchmark-derived masks, and Model F also pinned
its global magnitude to GFED5. This script records that prior work faithfully;
it does not promote the reproduced fields as clean candidates.
"""
...
PARAM_ABC = ROOT / "model" / "parameters" / "abc-gfed4.1s"
PARAM_GF5 = ROOT / "model" / "parameters" / "coupled-gfed5"
DEFAULT_OUTPUT = ROOT / "model" / "reproduced"
...
def historical_rescale(
    ...
    if model_mean <= 0:
        raise RuntimeError("model replay generated a non-positive mean")
...
def write_model(
    ...
    dataset = xr.Dataset(
        ...
        attrs={
            "title": f"ED-Fire model replay: {model_id}",
            "Conventions": "CF-1.7",
            "replay_protocol": protocol,
            "eligible_under_active_candidate_contract": "false",
            "warning": "This reproduced output uses benchmark-derived construction steps.",
        },
    )
...
def write_results(
    ...
    verification = {
        "schema": "ed-fire-model-replay/v1",
        "eligible_under_active_candidate_contract": False,
...
def parameter_inventory_bytes() -> bytes:
    rows: list[dict[str, Any]] = []
    for path in sorted((ROOT / "model" / "parameters").rglob("*")):
        if not path.is_file():
            continue
        row: dict[str, Any] = {
...
def write_parameter_inventory() -> Path:
    destination = ROOT / "model" / "parameter-inventory.csv"
    destination.write_bytes(parameter_inventory_bytes())
    return destination


def verify_parameter_inventory() -> Path:
    destination = ROOT / "model" / "parameter-inventory.csv"
    if not destination.is_file() or destination.read_bytes() != parameter_inventory_bytes():
        raise RuntimeError(
            "model/parameter-inventory.csv does not match model/parameters; "
            "run with --inventory-only to rebuild it deliberately"
        )
    return destination
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--evaluate",
        action="store_true",
        help="Run the pinned ILAMB executable and verify the recorded values.",
    )
    parser.add_argument(
        "--inventory-only",
        action="store_true",
...
def main() -> int:
    ...
    if args.inventory_only:
        ...
        print(f"Parameter inventory rebuilt: {inventory.relative_to(ROOT)}")
        return 0
    inventory = verify_parameter_inventory()
    print(f"Parameter inventory verified: {inventory.relative_to(ROOT)}")
    ...
    if not args.evaluate:
        ...
        print(
            "Generation complete. Reproduced target-fed outputs are not eligible "
            "under the active clean-candidate contract."
        )
        return 0
