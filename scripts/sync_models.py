...
BUNDLE = ROOT / "model" / "upstream" / "ed-autoresearch.bundle"
OUTPUT = ROOT / "model" / "commit-artifacts"
...
def archive_path(source_path: str, blob: str) -> str:
    ...
    relative = PurePosixPath("model/commit-artifacts/by-path")
...
def collect_history(repository: Path) -> tuple[
    ...
    coverage = {
        ...
        "history_bundle": "model/upstream/ed-autoresearch.bundle",
...
def expected_outputs(repository: Path) -> tuple[dict[str, bytes], dict[str, Any]]:
    ...
    outputs[
        "model/commit-artifacts/manifest.csv"
    ] = csv_bytes(MANIFEST_FIELDS, manifest)
    outputs[
        "model/commit-artifacts/model-attempts.csv"
    ] = csv_bytes(ATTEMPT_FIELDS, attempts)
    outputs[
        "model/commit-artifacts/coverage.json"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Expose and verify every committed ED-Fire model artifact."
    )
    parser.add_argument(
        "--write",
        action="store_true",
...
def main() -> int:
    ...
    if errors:
        print("Model artifact coverage: FAIL")
    ...
    print("Model artifact coverage: PASS")
    print(
