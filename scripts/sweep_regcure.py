"""Keep the 0.6783 regional-GDP model's gammas FIXED, add only curing, sweep cure_k.
Find the cure_k that improves the steppe without tanking aggregate ILAMB. Writes a BA
per cure_k (magnitude pinned to GFED) for scoring + prints the steppe/Australia response.
"""
import json, sys, os
...
g=lambda n,ds: np.nan_to_num(ds[n].isel(time=sl).values.astype(np.float32))
ds=xr.open_dataset("global_baseline_modelC_inputs_1997-2016.nc")
dbar=coarsen(g("D_bar",ds));t_air=coarsen(g("T_air",ds));p_ann=coarsen(g("P_ann",ds));p_month=coarsen(g("P_month",ds))
gpp=coarsen((np.clip(g("GPP_month_ntrl",ds),0,None)*g("area_frac_ntrl",ds)+np.clip(g("GPP_month_scnd",ds),0,None)*g("area_frac_scnd",ds)+np.clip(g("GPP_month_past",ds),0,None)*g("area_frac_past",ds)).astype(np.float32));ds.close()
...
p_ann_cell=p_ann.mean(0,keepdims=True);agb_cell=agb.mean(0,keepdims=True);cured=sig(dbar,0.001,3000.0)
CURE_UNIT=(1/(1+np.exp(-0.02*(p_ann_cell-F["cure_p_min"]))))*(1/(1+np.power(np.clip(agb_cell/F["cure_agb_crit"],0,None),3.0)))*(1/(1+gpp_cell/(F["cure_gpp_ref"]+1e-9)))*cured
...
gfield=gaussian_filter(gf,SIG,mode="nearest");M=np.clip(np.power(10.0,gfield*(W0-w)),0.15,6.0);M[~have]=1.0;M=M[None]
...
ba_from=lambda rate,s:(1-np.exp(-np.minimum(s*M*rate,5.0)/12.0)).astype(np.float32)
def s_pin(rate):
    lo,hi=0.02,8.0
    for _ in range(30):
        mid=0.5*(lo+hi)
        if float((ann(ba_from(rate,mid))*area*land).sum())<gtot:lo=mid
        else:hi=mid
    return 0.5*(lo+hi)
tot=lambda a,b:float((a*((LON>=b[0])&(LON<=b[1])&(LAT>=b[2])&(LAT<=b[3]))*area).sum())/1e10
ST=(48,90,45,56);AU=(112,154,-44,-10);AF=(-20,52,-36,18)
def write(b,name):
    ...
    d=add_cf_bounds(d);tu="days since 2001-01-01 00:00:00";os.makedirs(f"ilamb/MODELS_REGCURE_SWEEP/{name}",exist_ok=True)
    for s in __import__("pathlib").Path(f"ilamb/MODELS_REGCURE_SWEEP/{name}").glob("._*"):s.unlink()
    d.to_netcdf(f"ilamb/MODELS_REGCURE_SWEEP/{name}/burntArea.nc",encoding={"burntArea":{"zlib":True,"complevel":4,"_FillValue":1e20},"time":{"units":tu,"calendar":"noleap","dtype":"float64"},"time_bounds":{"units":tu,"calendar":"noleap","dtype":"float64"}},format="NETCDF4_CLASSIC")
print(f"GFED: steppe {tot(ann(gf5),ST):.0f}  Australia {tot(ann(gf5),AU):.0f}  Africa {tot(ann(gf5),AF):.0f}")
for ck in [0.0,0.1,0.2,0.35]:
    rate=R0+ck*CURE_UNIT;s=s_pin(rate);ba=ba_from(rate,s);nm=f"ED-ModelC-ck{int(ck*100):02d}";write(ba,nm)
    print(f"cure_k={ck:.2f} -> steppe {tot(ann(ba),ST):5.1f}  Australia {tot(ann(ba),AU):5.1f}  Africa {tot(ann(ba),AF):5.0f}   [{nm}]")
