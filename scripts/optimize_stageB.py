"""
optimize_stageB.py — Joint Optuna optimization of burnt-area + fFire for Model C.

Parameters searched (17 total):
  Burn (12): k1, D_low, k2, D_high, fire_exp, P_half, pre_dampen_half,
             gpp_af, gpp_b, gpp_d, ign_k, ign_c
  Combust (5): D_REF, BETA_MAX_leaf, BETA_MAX_fine, BETA_MAX_coarse, POOL_FRAC_fine
...
Output:
  models/C-stageB/params.json
  out_ffire_stageB/ED-ModelC-stageB-fFire/fFire.nc
  out_stageB/ED-ModelC-stageB/burntArea.nc
"""
...
OUT_MODEL = REPO / "models" / "C-stageB"
OUT_BA    = REPO / "out_stageB"   / "ED-ModelC-stageB"
OUT_FF    = REPO / "out_ffire_stageB" / "ED-ModelC-stageB-fFire"
...
def compute_ffire_1deg(pred_ba_1, d_ref, b_leaf, b_fine, b_coarse, pf_fine):
    """Compute fFire [kg/m²/s] at 1-deg from predicted burntArea + combust params."""
    ...
    C_combust = (C_leaf   * (b_leaf   * m_theta) +
...
def objective(trial: optuna.Trial) -> float:
    ...
    pred_ff = compute_ffire_1deg(pred_rescaled, d_ref, b_leaf, b_fine, b_coarse, pf_fine)
...
# Seed from current best Model C params to warm-start
warm_params = json.load(open(REPO / "models" / "C" / "params.json"))["params"]
warm_params.update({"D_REF": 1000.0, "BETA_MAX_leaf": 0.80, "BETA_MAX_fine": 0.75,
...
json_out = {
    "model":      "Model C-stageB (joint BA + fFire optimization)",
...
ds_ba = xr.Dataset(
    ...
    attrs={"title":"ED-Model C-stageB burntArea (joint-optimized)"})
...
ds_ff = xr.Dataset(
    ...
    attrs={"title":"ED-Model C-stageB fFire (joint-optimized, Stage B)",
           "method": "process-based; burntArea × Σ_k (C_k × β_k(θ)) / Δt",
...
print(f"  ED-Model C-stageB   fFire Overall = {ff_o:.4f}")
print(f"  ED-Model C-stageB   BA    Overall = {ba_o:.4f}  (prev C = 0.7369)")
