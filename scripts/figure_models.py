#!/usr/bin/env python3
"""Render comparable figures for the ED-Fire models A through I."""
...
DEFAULT_REPLAY = ROOT / "model" / "reproduced"
...
REGISTRY = ROOT / "model" / "registry.toml"
...
def load_metrics() -> dict[str, dict[str, float]]:
    if not METRICS.exists() or not VERIFICATION.exists():
        raise FileNotFoundError(
            "run scripts/reproduce_models.py --models all --evaluate first"
        )
    ...
    if failed:
        raise RuntimeError("unverified models: " + ", ".join(failed))
...
def save(plt: Any, figure: Any, path: Path) -> None:
    ...
    figure.savefig(
        path,
        dpi=DPI,
        metadata={"Software": "ED-Fire model replay"},
    )
    plt.close(figure)
...
def plot_scores(plt: Any, metrics: dict[str, dict[str, float]]) -> Path:
    ...
    axes[1].axhline(
        ...
        linestyle="--",
        linewidth=1.5,
        label=f"GFED5 ({reference_mha:.3f} Mha yr$^{{-1}}$)",
    )
    ...
    figure.suptitle(
        "ED-Fire models A-I: GFED5 scores and burned area, 2001-2016",
        fontsize=16,
    )
    path = OUTPUT / "models-a-i-gfed5-score-summary.png"
...
def plot_maps(
    ...
    panels: list[tuple[str, np.ndarray, str]] = [
        ("GFED5", reference, f"{reference_mha:.3f} Mha yr$^{{-1}}$")
    ]
    for model_id in MODEL_IDS:
        field, model_latitude, model_longitude = annual_percent(model_output(registry[model_id]))
        ...
        panels.append(
            (
                ...
                field,
                (
                    f"Overall {metrics[model_id]['overall_score']:.3f} · "
                    f"{metrics[model_id]['mha_per_year']:.3f} Mha yr$^{{-1}}$"
                ),
            )
        )

    figure, axes = plt.subplots(5, 3, constrained_layout=True)
    image = None
    ...
    if unused:
        ...
        boundary.text(
            ...
            (
                ...
                "These outputs do not satisfy the active clean-candidate contract."
            ),
    ...
    path = OUTPUT / "models-a-i-gfed5-mean-maps.png"
    save(plt, figure, path)
    return path
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Render fixed comparisons and canonical suites for models A through I."
    )
    parser.add_argument(
        "--models",
        nargs="+",
        default=["Ibest"],
        metavar="MODEL",
        help="Model IDs to render, or 'all' (default: Ibest).",
    )
    parser.add_argument(
        "--replay-root",
        type=Path,
        default=DEFAULT_REPLAY,
        help="Root containing replayed model outputs and metrics.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=None,
        help="Figure root; defaults to <replay-root>/figures.",
    )
...
def model_output(model: dict[str, Any]) -> Path:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"missing replay output for {model['id']}; run reproduce_models.py first"
        )
    return path
...
def evaluate_model(
    ...
    cache_record.write_text(
        json.dumps(
            {
                "schema": "ed-fire-model-standardized-evaluation/v1",
                "signature": signature,
                "eligible_under_active_candidate_contract": False,
...
def write_suite_record(
    ...
    record = {
        "schema": "ed-fire-model-figure-suite/v1",
        "model_id": model["id"],
...
def configure_paths(replay_root: Path, output_root: Path | None) -> None:
    global REPLAY, METRICS, VERIFICATION, OUTPUT, EVALUATIONS

    replay = replay_root.resolve()
    output = output_root.resolve() if output_root is not None else replay / "figures"
    for label, path in (("--replay-root", replay), ("--output", output)):
        try:
            path.relative_to(ROOT)
        except ValueError as error:
            raise ValueError(f"{label} must remain inside the ED-Fire repository") from error
    REPLAY = replay
    METRICS = REPLAY / "metrics.csv"
    VERIFICATION = REPLAY / "verification.json"
    OUTPUT = output
...
def main() -> int:
    args = parse_args()
    configure_paths(args.replay_root, args.output)
    registry = model_registry()
    selected = select_models(args.models, registry)
    metrics = load_metrics()
    plt = plotting()
    for path in (plot_scores(plt, metrics), plot_maps(plt, metrics, registry)):
    ...
    for model_id in selected:
        ...
        for path in paths:
            print(path.relative_to(ROOT))
    print(
        "Model figures reproduce prior work; target-fed outputs are not clean-candidate evidence."
    )
    return 0
