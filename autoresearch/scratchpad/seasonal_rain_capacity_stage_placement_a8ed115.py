"""Test where rain-built seasonal fuel capacity enters the operator stack."""
...
def predict_at_stage(data: dict[str, np.ndarray], insertion: str) -> np.ndarray:
    ...
    if insertion == "transform":
        prediction = model._seasonal_rainfall_capacity(
            prediction, data, params, enabled
        )
        inserted = True
    for label, function in STAGES:
        if label == "seasonal_rain_capacity":
            continue
...
def main() -> int:
    ...
    incumbent = all_scores["regime_capacity"]
